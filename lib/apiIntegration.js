...
class LaunchDarkly_API {
    ...
    async postEvaluateFlagsForContextInstance(projectKey, environmentKey, jsonBody, additionalFilters = '') {
        return await post(
            this.url,
            `projects/${projectKey}/environments/${environmentKey}/flags/evaluate${
                additionalFilters ? `?${additionalFilters}` : ''
            }`,
            jsonBody,
            this.headers,
